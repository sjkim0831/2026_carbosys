 ...
 *               수정일 수정자 수정내용
 *               ---------- ------- -------------------
 *               2019.11.29 신용호 encryptPassword(String data) 삭제 : KISA 보안약점 조치
 *               (비밀번호 해시함수 적용 시 솔트를 사용하여야 함)
    ...
    try {
      if (srcFile.exists() && srcFile.isFile()) {
        ...
        output = new BufferedOutputStream(
            new FileOutputStream(EgovWebUtil.filePathBlackList(STORE_FILE_PATH + FilenameUtils.getName(target))));
